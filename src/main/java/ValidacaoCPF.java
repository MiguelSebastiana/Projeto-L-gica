public class ValidacaoCPF {
    ...
    public static void validar(String cpf) {
        if (cpf == null || cpf.trim().isEmpty()) {
            throw new ValidatorCPF("ERRO - CPF NÃO PODE SER VAZIO!");
        }
        ...
        if (cpfNumerico.length() != 11 || cpfNumerico.matches("(\\d)\\1{10}")) {
            throw new ValidatorCPF("ERRO - CPF DE TAMANHO INVÁLIDO OU SEQUÊNCIA DE DÍGITOS IGUAIS!");
        }
        ...
        if (cpfNumerico.charAt(9) != (d1 + '0') || cpfNumerico.charAt(10) != (d2 + '0')) {
            throw new ValidatorCPF("ERRO - CPF INVÁLIDO (DÍGITOS VERIFICADORES INCORRETOS)!");
        }
    }
